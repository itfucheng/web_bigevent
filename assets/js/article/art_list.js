$(function () {
  ...
  //定义查询的参数对象，将来请求数据的时候，需要将请求参数对对象提交到服务器
  var q = {
  ...
  function renderPage(total) {
    ...
    laypage.render({
      ...
      jump: function(obj, f) {  //f 可以是任何一个变量
        // console.log(obj.curr);
        // console.log(f);
        //把最新的页码值，赋值到q这个查询参数对象
        q.pagenum = obj.curr
    ...
    layer.confirm('确认删除？', {icon: 3, title:'提示'}, function(index){
      ...
      $.ajax({
        ...
        success: function (res) {
          ...
          //如果没有剩余的数据了，则让页码值减 1  之后 再重新调用initTable 方法
          if (len === 1) {
...
//解决删除分页更新的另一种思路
// 定义一个data对象，一个变量为len 再包含一个q对象
// let data = { len: '', q:{}}

//在initTable方法里给len赋值 data.len = res.data.length
// 最后在删除的时候用len去判断是否为当前页面左后一条数据